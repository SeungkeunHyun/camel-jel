!function (name, definition) {
	...
	Jel.fn = Jel.prototype = {
		...
		xmlImport: function (wrapper, baseEl) {
			...
			var setAttrs = function (props, element) {
				console.log("setAttr", element, props);
				for (var propName in props) {
					props[propName] = null;
					console.log(propName, "is array?", props[propName] instanceof Array)
					if (props[propName] instanceof Array) {
						...
						if (element.childNodes && element.childNodes.length > 0) {
							...
							for (z = 0; z < element.childNodes.length; z++) {
								console.log(element.childNodes[z]);
								if (element.childNodes[z].nodeName == propName) {
									if (lement.childNodes[z].children && lement.childNodes[z].children.length > 0) {
									...
									else if (element.childNodes[z].innerHTML) {
										props[propName] = lement.childNodes[z].innerHTML.trim()
									}
									else if (element.childNodes[z].childNodes && element.childNodes[z].childNodes.length) {
										//if the element has a number of child greater than 1, it can cointains some other elements
										if (element.childNodes[z].childNodes.length > 1) {
											setAttrs(props[propName], element[z].childNodes)
										}
										else props[propName] = element.childNodes[z].childNodes[0].nodeValue.trim();
									}
								} 
							}
						}
							...
							var z;
							for (z = 0; z < element.attributes.length; z++) {
								//if(!props[element.attributes[z].name])
								props[element.attributes[z].name] = element.attributes[z].nodeValue;
							}
						}
					}
				}
				[...element.getElementsByTagName('*')].forEach(el => {
					console.log('child node', el);
					props[el.nodeName] = el.innerHTML;
				});
			...
			console.log("root element", rootEl, wrapper);
			if (rootEl.length) {
				var i = 0;
				for (i = 0; i < rootEl.length; i++) {
					//if a baseEl exists, then we have to start converting from it 
					if (baseEl) {
						...
						for (j = 0; j < rootEl[i].childNodes.length; j++) {
							console.log('namespace check', rootEl[i].childNodes[j].tagName, rootEl[i].childNodes[j].namespaceURI, this.getNamespace());
							if (rootEl[i].childNodes[j].namespaceURI == this.getNamespace()) {
								elements.push(rootEl[i].childNodes[j])
							}
						}
						let prev_instance, curr_instance = null;
						for (j = 0; j < elements.length; j++) {
							//we can create here the shape and adjust its attributes/innerTag
							var k, curr_shape;
							for (k = 0; k < Jel.paletteShapes.length; k++) {
								console.log('shape metaelement', k, Jel.paletteShapes.at(k).metaelement);
								if (Jel.paletteShapes.at(k).metaelement == 'process') {
									console.log("find processor", elements[j].nodeName, processors.find(tag => tag == elements[j].nodeName));
								}
								if (Jel.paletteShapes.at(k).metaelement == elements[j].nodeName) {
									//curr_shape = $.extend(true, {}, Jel.paletteShapes.at(k));
									curr_shape = _.clone(Jel.paletteShapes.at(k));
									if (curr_shape) curr_shape.id = starting_id++;
									break;
								} else if (Jel.paletteShapes.at(k).metaelement == 'process' && processors.find(tag => tag == elements[j].nodeName) != null) {
									curr_shape = _.clone(Jel.paletteShapes.at(k));
									curr_shape.metaelement = elements[j].nodeName;
									if (curr_shape) curr_shape.id = starting_id++;
									break;
								}
							}
							setAttrs(curr_shape.props, elements[j]);
							console.log('shape props', j, curr_shape.props);
							...
							curr_instance = Jel.Canvas.addShape(curr_shape);
							if (prev_instance != null) {
								Jel.Canvas.connect(prev_instance, curr_instance);
							}
	...
	Jel.getBaseElement = Jel.fn.getBaseElement;
	Jel.setXMLWrapper = Jel.fn.setXMLWrapper;
	Jel.getXMLWrapper = Jel.fn.getXMLWrapper;
	...
	Jel.getNamespace = Jel.fn.getNamespace;
